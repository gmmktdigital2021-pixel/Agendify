...
import { CalendarCheck, Clock, Zap, Star, ChevronDown, ChevronUp, MessageCircle, Menu } from "lucide-react";
...
export default function LandingPage() {
  ...
  const nichos = [
    ...
    { label: "Estéticas e Spa", href: "/nichos/estetica", emoji: "🧖" },
  ...
  const faqs = [
    ...
    { q: "Como meus clientes fazem o agendamento?", a: "Você recebe um link público personalizado para compartilhar no Instagram ou WhatsApp. O cliente acessa, escolhe o serviço e o horário e pronto!" },
    ...
    <div className="font-sans bg-white min-h-screen">
      <nav className="border-b border-slate-100 bg-white sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            <div className="relative">
              <button onClick={() => setNichosOpen(!nichosOpen)} className="flex items-center gap-1 text-slate-600 hover:text-brand font-medium text-sm">
              ...
              {nichosOpen && (
                <div className="absolute top-8 left-0 bg-white border border-slate-200 rounded-xl shadow-xl p-3 w-56 z-50">
                  {nichos.map(n => (
                    <Link key={n.href} href={n.href} onClick={() => setNichosOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-brand/5 hover:text-brand text-sm text-slate-700">
            ...
            <a href="#funcionalidades" className="text-slate-600 hover:text-brand font-medium text-sm">Funcionalidades</a>
            <a href="#precos" className="text-slate-600 hover:text-brand font-medium text-sm">Preços</a>
            <a href="#contato" className="text-slate-600 hover:text-brand font-medium text-sm">Contato</a>
          </div>
          <div className="hidden md:flex items-center gap-3">
            <Link href="/login" className="text-slate-600 hover:text-brand font-medium text-sm">Entrar</Link>
            <Link href="/login" className="bg-brand text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-purple-700">Começar grátis</Link>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-100 p-4 flex flex-col gap-3">
            {nichos.map(n => <Link key={n.href} href={n.href} className="text-slate-600 text-sm py-1">{n.emoji} {n.label}</Link>)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h1 className="text-5xl sm:text-6xl font-extrabold tracking-tight text-slate-900 leading-tight mb-6">
              Sua agenda no <br /><span className="text-brand">piloto automático.</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              <Link href="/login" className="bg-brand text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-purple-700 transition-all shadow-lg shadow-brand/30">Começar grátis</Link>
              <a href="#funcionalidades" className="border-2 border-slate-200 text-slate-700 px-8 py-4 rounded-xl font-bold text-lg hover:border-brand hover:text-brand transition-all">Ver funcionalidades</a>
            </div>
            <p className="text-slate-500 text-sm">+500 profissionais já usam o Agendify</p>
          </div>
          <div className="relative">
            <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs text-slate-400 ml-2">agendify.app/dashboard</span>
              </div>
                ...
                {[["Ana Clara","Corte + Escova","09:00","confirmado"],["Mariana","Coloração","11:00","pendente"],["Joana","Manicure","14:00","confirmado"]].map(([nome,servico,hora,status],i) => (
                  <div key={i} className="flex items-center justify-between bg-slate-50 rounded-lg p-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-brand/20 flex items-center justify-center text-brand font-bold text-xs">{nome[0]}</div>
                      <div>
            ...
            <div className="absolute -top-4 -right-4 bg-green-500 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg">✅ Reserva confirmada!</div>
            <div className="absolute -bottom-4 -left-4 bg-brand text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg">⏱ 20h/semana economizadas</div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-6 border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6 text-slate-400 font-bold text-sm">
            {["Salões","Barbearias","Nail Designers","Lash Designers","Maquiadoras","Estéticas"].map(n => <span key={n}>• {n}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {nichos.map(n => (
            <Link key={n.href} href={n.href} className="group bg-white border-2 border-slate-100 rounded-2xl p-6 text-center hover:border-brand hover:shadow-xl transition-all duration-300">
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-slate-600 max-w-2xl mx-auto">Esqueça as ligações, as mensagens no WhatsApp às 23h e os clientes que somem sem avisar.</p>
          </div>
      ...
      <section id="funcionalidades" className="py-24 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className="bg-white rounded-2xl p-6 border border-slate-200 hover:border-brand hover:shadow-lg transition-all duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          <div className="bg-white rounded-2xl border-2 border-slate-200 p-8 hover:-translate-y-2 transition-all duration-300">
            ...
            {[["✅","Até 30 agendamentos/mês"],["✅","Até 20 clientes"],["✅","Até 5 serviços"],["✅","Link público de agendamento"],["✅","Integração WhatsApp"],["❌","Relatórios e gráficos"],["❌","Clientes ilimitados"],["❌","Suporte prioritário"]].map(([icon,text],i) => (
              <div key={i} className="flex items-center gap-2 mb-3 text-sm"><span>{icon}</span><span className={icon === "❌" ? "text-slate-400" : "text-slate-700"}>{text}</span></div>
            ))}
          ...
          <div className="bg-brand rounded-2xl border-2 border-brand p-8 hover:-translate-y-2 transition-all duration-300 relative shadow-2xl shadow-brand/30">
            ...
            {[["✅","Agendamentos ilimitados"],["✅","Clientes ilimitados"],["✅","Serviços ilimitados"],["✅","Link público de agendamento"],["✅","Integração WhatsApp"],["✅","Relatórios e gráficos"],["✅","Dashboard completo"],["❌","Suporte prioritário"]].map(([icon,text],i) => (
              <div key={i} className="flex items-center gap-2 mb-3 text-sm"><span>{icon}</span><span className={icon === "❌" ? "text-purple-300" : "text-white"}>{text}</span></div>
            ))}
          ...
          <div className="bg-white rounded-2xl border-2 border-slate-200 p-8 hover:-translate-y-2 transition-all duration-300">
            ...
            {[["✅","Tudo do Profissional"],["✅","Múltiplos profissionais"],["✅","Suporte prioritário"],["✅","Personalização avançada"],["✅","Relatórios avançados"],["✅","Acesso antecipado a novidades"]].map(([icon,text],i) => (
              <div key={i} className="flex items-center gap-2 mb-3 text-sm"><span>{icon}</span><span className="text-slate-700">{text}</span></div>
            ))}
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <button onClick={() => setFaqOpen(faqOpen === i ? null : i)} className="w-full flex items-center justify-between p-6 text-left hover:bg-slate-50">
                ...
                {faqOpen === i && <div className="px-6 pb-6 text-slate-600 border-t border-slate-100 pt-4">{faq.a}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-gradient-to-r from-purple-700 to-brand text-white text-center">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <Link href="/login" className="bg-white text-brand px-10 py-4 rounded-xl font-extrabold text-lg hover:bg-purple-50 transition-all shadow-xl inline-block">Criar minha agenda grátis</Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} required placeholder="Seu nome" className="w-full px-4 py-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-brand text-slate-800" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">WhatsApp</label>
              <input value={formData.whatsapp} onChange={e => setFormData({...formData, whatsapp: e.target.value})} required placeholder="11999990000" className="w-full px-4 py-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-brand text-slate-800" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1">Email</label>
            <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} required placeholder="seu@email.com" className="w-full px-4 py-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-brand text-slate-800" />
          </div>
          <div>
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-brand text-white px-6 py-3 rounded-xl font-bold shadow-2xl z-50">
          {toast}
        </div>
      )}
    </div>
