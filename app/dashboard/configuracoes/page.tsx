...
// Mapeia texto do select → minutos (integer para o banco)
...
// Mapeia minutos → texto (para exibir no select)
...
export default function ConfiguracoesPage() {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>, type: 'perfil' | 'capa') => {
    try {
      ...
      setToastMsg(`✅ Foto de ${type} atualizada com sucesso!`);
    } catch (err: any) {
      console.error(err);
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {salonId && (
          <Card className="p-6 border-brand/20 bg-brand/5 shadow-[0_4px_20px_-4px_rgba(124,58,237,0.1)]">
            ...
            <p className="text-sm text-slate-600 mb-4">
              Clientes podem acessar este link para agendar um horário sem precisar ligar ou contatar você previamente.
            </p>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-end mb-8">
            <Input type="time" label="Abertura" value={formData.horario_inicio}
              onChange={(e) => setFormData({ ...formData, horario_inicio: e.target.value })} required />
            <Input type="time" label="Fechamento" value={formData.horario_fim}
              onChange={(e) => setFormData({ ...formData, horario_fim: e.target.value })} required />
            <Input type="time" label="Início do Almoço" value={formData.horario_almoco_inicio}
              onChange={(e) => setFormData({ ...formData, horario_almoco_inicio: e.target.value })} />
            <Input type="time" label="Fim do Almoço" value={formData.horario_almoco_fim}
          ...
          <div className="pt-2 border-t border-slate-100">
            ...
            <div className="flex flex-wrap gap-2 md:gap-3">
              {(Object.keys(activeDays) as Array<keyof typeof activeDays>).map(key => {
                ...
                return (
                  <label key={key} className={`flex items-center justify-center px-4 md:px-5 py-2 border rounded-[12px] cursor-pointer transition-all user-select-none text-[13px] ${isActive ? 'bg-brand/10 border-brand text-brand font-bold shadow-sm' : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                    <input type="checkbox" className="hidden" checked={isActive} onChange={() => handleToggleDay(key)} />
                    {dayLabels[key]}
                  </label>
                );
              })}
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              >
                {Object.keys(antecedenciaMap).map(opt => (
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:bg-white focus:outline-none focus:border-brand font-semibold text-slate-700 transition-colors cursor-pointer"
              >
                {Object.keys(intervaloMap).map(opt => (
                  <option key={opt} value={opt}>{opt}</option>
                ))}
              </select>
            </div>
        ...
        <Card className="p-6">
          ...
          <div>
            <div className="flex flex-wrap gap-2 mb-3">
              {['{nome}', '{dia}', '{hora}', '{servico}', '{valor}'].map(v => (
                <button type="button" key={v} onClick={() => insertVariable(v)}
                  className="px-3 py-1 bg-brand/10 text-brand text-[11px] font-bold rounded-lg hover:bg-brand hover:text-white transition-colors">
                  {v}
                </button>
              ))}
            </div>
            ...
            <div className="flex justify-end mb-4">
              <button type="button" onClick={() => setFormData({ ...formData, mensagem_padrao: WHATSAPP_DEFAULT })}
                className="text-[12px] font-bold text-slate-400 hover:text-brand transition-colors">
                Restaurar padrão
              </button>
            </div>
        ...
        <Card className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
            ...
            <div className="flex-1 w-full bg-slate-50 py-5 rounded-xl border border-slate-100 flex flex-col items-center justify-center">
              <button type="button"
                className="px-10 py-3 text-white text-sm font-bold rounded-lg shadow-lg transition-colors cursor-default"
